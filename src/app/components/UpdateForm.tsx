...
function UpdateForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (formRef.current && entryId) {
      // Assuming entryId is passed as a prop
      const formData = new FormData(formRef.current);
      await updateEntry(props.entryId, formData); // Make sure to pass the entry ID here
      formRef.current.reset();
      // Optionally redirect the user back to the entries list or show a success message
    }
  };
